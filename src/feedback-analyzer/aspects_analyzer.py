...
for review in reviews:
    ...
    # Собираем с маппингом на аспекты
...
# --- Разделение по оценке ---
negative_reviews = []
positive_or_neutral_reviews = []
...
for row in result_rows:
    ...
    if score in [1, 2]:
        ...
        positive_or_neutral_reviews.append(row)

# --- Сохраняем ---
with open("negative_reviews.json", "w", encoding="utf-8") as f_neg:
    json.dump(negative_reviews, f_neg, ensure_ascii=False, indent=2)

with open("positive_reviews.json", "w", encoding="utf-8") as f_pos:
    json.dump(positive_or_neutral_reviews, f_pos, ensure_ascii=False, indent=2)

print("Готово! Отзывы разбиты на два файла:")
print("  → негативные: negative_reviews.json")
print("  → позитивные/нейтральные: positive_reviews.json")
